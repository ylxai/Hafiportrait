...
export default function Logo({ 
  ...
  const sizeConfig = {
    sm: {
      ...
      underlineWidth: 'w-16 sm:w-20 md:w-24',
      underlineLeft: 'left-3 sm:left-4'
    },
    md: {
      h: 'text-xl sm:text-2xl md:text-3xl lg:text-4xl',
      underlineWidth: 'w-20 sm:w-24 md:w-28 lg:w-32',
    ...
    xl: {
      ...
      underlineWidth: 'w-28 sm:w-32 md:w-36 lg:w-40 xl:w-44',
  ...
  return (
    <div className={`flex items-center group cursor-pointer relative ${className}`}>
      {/* Logo Container with unified text */}
      <div className="flex items-baseline relative mobile-logo-container">
        {/* "Hafiportrait" as single unified text */}
        <span 
          className={`${config.h} text-gray-800 dark:text-gray-100 group-hover:text-gray-700 dark:group-hover:text-gray-200 group-hover:scale-105 transition-all duration-700 relative z-10 mobile-logo-glow ${
            animated && isLoaded 
