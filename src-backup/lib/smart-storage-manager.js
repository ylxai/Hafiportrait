...
class SmartStorageManager {
  ...
  async compressImage(photoFile, compressionType) {
    ...
    try {
      ...
      console.log(`🎯 Compressing with Sharp: ${compressionType} (quality: ${Math.round(settings.quality * 100)}%, maxWidth: ${settings.maxWidth}px)`);
      ...
      const compressionRatio = ((photoFile.size - compressedBuffer.length) / photoFile.size * 100).toFixed(1);
      console.log(`✅ Compression successful: ${photoFile.size} → ${compressedBuffer.length} bytes (${compressionRatio}% reduction)`);
      ...
      console.warn('⚠️ Sharp not available, using fallback compression:', error.message);
      
      // Enhanced fallback: try to use fallback compression module
      try {
        const createFallbackCompression = require('./fallback-compression');
        const fallbackCompressor = createFallbackCompression();
        
        console.log(`🔄 Using fallback compression for ${compressionType}`);
        const result = await fallbackCompressor.compressImage(photoFile, settings);
        ...
        console.warn('⚠️ Fallback compression also failed:', fallbackError.message);
